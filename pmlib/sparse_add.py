...
# FIXME: Type checking everything is a waste of computing resources, range check is ok.
...
def sptoeplitz(col, *args):
    ...
    if len(args) == 0:
        col[0] = col[0].conjugate()
        row = col.copy()
        col = np.conjugate(col)
    else:
        ...
        if col[0] != row[0]:
            warn(
                'first element of input column does not match first'
...
def spdistr(ymax, x0, N, p='trunc', eps=1e-09):
    ...
    try:
        ...
        if any([not 0 <= elm <= 1 for elm in x0]):
            raise ValueError
    ...
    try:
        ...
        if any([not elm > 3 for elm in N[0:-1]]):
            raise ValueError
        float(N[-1])  # FIXME: It is better to check isinstance(x, float).
        if not N[-1] > 0:
    ...
    if not dim:
        ...
        if p == 'trunc' or alpha0 < eps:
            ...
            E[l0,0] = (1 - alpha0) * ymax
            E[l0+1,0] = alpha0 * ymax
        else:
...
def spdistr1D_diag(ymax, x0, N, p='trunc', eps=1e-09):
    ...
    if p == 'trunc' or alpha0 < eps:
        ...
        E[l0,l0] = (1 - alpha0) * ymax
        E[l0 + 1,l0 + 1] = alpha0 * ymax
    else:
...
def spdistr2D_diag(zmax, x0, y0, Nx, Ny, epsilon=1, p='trunc', eps=1e-09):
    ...
    J[:Ny + 1,:Ny + 1] = E
    for i in range(1,Nx):
        J[i*(Ny + 1) : (i + 1)*(Ny + 1), i*(Ny + 1) : (i + 1)*(Ny + 1)] = E
    J[-Ny - 1:,-Ny - 1:] = E
